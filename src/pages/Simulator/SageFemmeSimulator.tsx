  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Paper elevation={3} sx={{ p: isMobile ? 2 : 4, borderRadius: 2 }}>
        <Typography variant={isMobile ? "h5" : "h4"} component="h1" gutterBottom align="center" sx={{ mb: 4 }}>
          Simulateur de Comparaison BNC vs ODIA Stratégie
        </Typography>

        {/* Formulaire de saisie du CA */}
        <form onSubmit={handleSubmit}>
          <TextField fullWidth label="Chiffre d'affaires annuel" variant="outlined" type="number" value={brut} onChange={(e) => setBrut(e.target.value)} placeholder="Entrez votre CA" InputProps={{ startAdornment: <Box component="span" sx={{ mr: 1 }}>€</Box>, }} sx={{ mb: 3 }} />
          ...
          <Box textAlign="center" sx={{ mb: 3 }}>
            ...
            <Button variant="outlined" color="info" size="large" onClick={() => navigate("/regles-de-calcul")} sx={{ ml: 2 }}>
              Règles de Calcul
            </Button>
          </Box>
        </form>
...
export default SageFemmeSimulator;
